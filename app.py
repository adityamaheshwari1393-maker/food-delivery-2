...
with tabs[5]:
    ...
    with c1:
        ...
        if len(freq_items) == 0:
            ...
            rules_disp = rules[["antecedents","consequents","support","confidence","lift"]].head(30).copy()
            rules_disp["antecedents"] = rules_disp["antecedents"].astype(str)
            rules_disp["consequents"] = rules_disp["consequents"].astype(str)
            rules_disp["support"]    = pd.to_numeric(rules_disp["support"],    errors="coerce").round(3)
            rules_disp["confidence"] = pd.to_numeric(rules_disp["confidence"], errors="coerce").round(3)
            rules_disp["lift"]       = pd.to_numeric(rules_disp["lift"],       errors="coerce").round(3)
    ...
    if len(freq_items) > 0 and len(rules) > 0:
        ...
        rules_plot = rules.head(50).copy()
        rules_plot["lift"] = pd.to_numeric(rules_plot["lift"], errors="coerce")
        rules_plot["support"] = pd.to_numeric(rules_plot["support"], errors="coerce")
        rules_plot["confidence"] = pd.to_numeric(rules_plot["confidence"], errors="coerce")
        fig = px.scatter(rules_plot, x="support", y="confidence",
